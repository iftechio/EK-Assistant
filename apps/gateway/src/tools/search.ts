...
/** 达人特征标签 → backend 布尔字段映射；no_ 前缀表示要求不具备该特征 */
const ATTRIBUTE_FIELD_MAP = {
  face_on_camera: { field: 'faceOnCamera', value: true },
  no_face_on_camera: { field: 'faceOnCamera', value: false },
  voice_over: { field: 'voiceOver', value: true },
  no_voice_over: { field: 'voiceOver', value: false },
  vertical_account: { field: 'isVertical', value: true },
  no_vertical_account: { field: 'isVertical', value: false },
  couple: { field: 'isCouple', value: true },
  no_couple: { field: 'isCouple', value: false },
  has_kids: { field: 'hasKids', value: true },
  no_kids: { field: 'hasKids', value: false },
  has_pets: { field: 'hasPets', value: true },
  no_pets: { field: 'hasPets', value: false },
  tk_shop: { field: 'hasTkShop', value: true },
  no_tk_shop: { field: 'hasTkShop', value: false },
  amz_partner: { field: 'isAmzPartner', value: true },
  no_amz_partner: { field: 'isAmzPartner', value: false },
} as const

const ATTRIBUTE_TAGS = Object.keys(ATTRIBUTE_FIELD_MAP) as [
  keyof typeof ATTRIBUTE_FIELD_MAP,
  ...(keyof typeof ATTRIBUTE_FIELD_MAP)[],
]
...
const searchKolsInput = z.object({
  platform: z.enum(DISCOVERY_PLATFORMS).describe('单个平台；多平台需求拆成多次调用'),
  ...
  attributeTags: z
    .array(z.enum(ATTRIBUTE_TAGS))
    .optional()
    .describe('达人特征要求（多选）；no_ 前缀表示要求不具备该特征'),
  demographics: z
    .object({
      gender: z.string().optional().describe('如 female/male 或 backend 支持的代码'),
      ageRange: z.string().optional().describe('如 18-24 或 backend 支持的代码'),
      skinColor: z.string().optional().describe('肤色/族裔标签代码'),
    })
    .optional()
    .describe('达人人口属性要求'),
  category: z
    .object({
      main: z.string().optional().describe('主垂类标签代码'),
      sub: z.string().optional().describe('子垂类标签代码'),
...
export type SearchKolsInput = z.infer<typeof searchKolsInput>

/** 工具参数 → backend /api/search/v2/web-search 请求体（纯函数，映射口径见测试） */
export function buildSearchBody(input: SearchKolsInput, projectId: string): Record<string, unknown> {
  const attrs: Record<string, boolean> = {}
  for (const tag of input.attributeTags ?? []) {
    const m = ATTRIBUTE_FIELD_MAP[tag]
    if (m) attrs[m.field] = m.value
  }
  return {
    projectId,
    ...
    minSubscribers: input.followers?.min,
    maxSubscribers: input.followers?.max,
    minVideosAverageViews: input.averageViews?.min,
    maxVideosAverageViews: input.averageViews?.max,
    minAverageLikeCount: input.averageLikes?.min,
    maxAverageLikeCount: input.averageLikes?.max,
    hasContactInfo: input.hasContactInfo,
    gender: input.demographics?.gender,
    ageRange: input.demographics?.ageRange,
    skinColor: input.demographics?.skinColor,
    mainCategory: input.category?.main,
    subCategory: input.category?.sub,
    tone: input.category?.tone,
    ...
    batchCount: getBatchCount(input),
...
export const searchKols = defineTool({
  ...
  execute: async (input, ctx) => {
    ...
    const task = await ctx.backend.post<TaskDetail>(
