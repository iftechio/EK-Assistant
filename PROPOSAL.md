...
- **第一期渠道**：先做Web对话界面
- **MVP三大场景**：KOL搜索与相似发现、邮件outreach自动化、任务与数据追踪（+场景5：评论反馈分析与效果归因）。~~质量筛选/打分~~已砍掉（产品决策：不做）
...
5. ~~质量分（Quality Blogger）~~ **已砍掉（产品决策：EK-Assistant不做质量筛选场景）**。背景留档：判定逻辑在`src/services/kolAiTagging/quality.ts`的`validateKolQuality()`，无对外接口，若做需backend包一个只读接口——现已无此需求。**MVP需要backend配合的改动只剩一块：场景5的评论拉取/导出接口（见场景5）**，其余全部复用现存接口。
...
9. ~~全自动找博主（`/api/copilot`托管任务）~~ **已砍掉（产品决策：EK-Assistant不接入此场景）**。背景留档：backend已有`/api/copilot`接口（web端`/auto-task`的后端，托管式自动找人，支持stop/resume/轮询，`routes/api/copilot.ts`），若日后要接入是零backend改动的封装工作。
...
EK-Assistant Gateway (Node/TS, Fastify)
  ...
  └─ Scheduler (MVP: 1个每日摘要recipe) —— 参考openclaw的定时能力，用其同款的croner库做进程内cron调度（轻量无依赖，不引入调度框架）；结果投递与失败通知参考openclaw cron/delivery-plan（定时任务产出可靠回投到对话、失败时通知用户，而不是静默丢失）
        │ Authorization: Bearer <user's Supabase JWT>  （原样转发，像多一个前端一样）
talent-marking-backend  /api/*  （不改动，除场景5的评论拉取/导出接口）
```
...
| 质量筛选（优质博主判定） | D | 产品决策：EK-Assistant不做此场景 |
...
| 全自动找博主（`/auto-task`） | D | 产品决策：EK-Assistant不接入此场景（backend `/api/copilot`接口现成，日后若要接入是零backend改动的封装工作） |
...
**结论**：MVP的3+1场景加上补入的`save_kols_to_project`零成本工具后，Web端"找人→存人→建联→追踪"的高频完整工作流全部覆盖；B类均为接口现成的低成本迭代项；C类是有意保留的手动环节；D类给出明确理由。后续验收"是否涵盖所有已知功能"以本矩阵为准。

## 参考项目技术栈对照（claude-code / openclaw 实际用了什么，我们采纳什么）

| 技术点 | claude-code | openclaw | EK-Assistant 决策 |
|---|---|---|---|
| 语言/运行时 | TypeScript + Bun | TypeScript + Node ≥22 + ESM + pnpm | **TypeScript + Node 22 + ESM + pnpm**（与backend一致，不用Bun——backend生态是Node，没必要引入第二运行时） |
| HTTP框架 | 无（CLI程序） | Hono + ws | **Fastify**（openclaw选Hono是因为要跑在多种边缘环境；我们只部署Node服务端，Fastify与backend同栈、团队熟悉，收益更大） |
| 模型层 | Anthropic SDK直连（单一供应商产品） | **pi-ai多供应商抽象库** | **Vercel AI SDK**——openclaw"不直连厂商、走多供应商抽象"的思路与我们一致，选AI SDK而非pi-ai是因为backend已在用同款 |
| 工具schema | Zod v4 | Zod v4 + TypeBox | **Zod**（AI SDK原生支持；backend路由的TypeBox仅在生成client时对接） |
| 流式回传 | 终端内渲染 | WebSocket (ws) | **SSE优先**（单向流式对话够用、比WS简单；确认卡片等交互走REST回传） |
| 定时任务 | CronCreate工具体系 | **croner库** | **采纳croner**：MVP的每日摘要recipe用croner做进程内调度，替代原方案"手写30秒轮询loop"，更省心且支持cron表达式 |
| 测试 | — | vitest 4 | **vitest**（与前端生态统一，替代jest） |
| 会话/记忆存储 | 磁盘JSONL会话 + memdir记忆目录 + 自动压缩 | sqlite-vec向量记忆 | MVP用**Postgres存会话**（公司已有RDS），轻量记忆按(user,project)存表；向量检索类记忆留到后续 |
| 可观测 | OpenTelemetry + 遥测 | tslog + 健康监控 | 复用backend现有的Sentry/监控惯例，MVP不引入OTel |
| 扩展机制 | MCP + 插件 | MCP + plugin-sdk + Skills | MVP不做（覆盖矩阵已定）；工具注册表保持"名字+schema+handler"的可扩展形状即可 |

结论：两个参考项目共同验证了我们的三个核心选型——TypeScript全栈、模型层走多供应商抽象不绑厂商、工具用Zod schema声明；差异点（Bun/Hono/pi-ai）都源于它们各自的部署形态，不适用于"和talent-marking-backend同机房部署的Node服务"这个场景，逐项给出了取舍理由。
...
- 质量分/优质博主判定与筛选（产品决策：不做，backend也无需加质量分接口）
- 全自动找博主托管任务（产品决策：不接入backend的`/api/copilot`）
