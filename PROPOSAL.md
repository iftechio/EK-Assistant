...
5. **质量分（Quality Blogger）没有独立对外接口**，判定逻辑在`src/services/kolAiTagging/quality.ts`的`validateKolQuality()`，是唯一真源（核查确认：全仓只有两处内部调用，无任何路由引用）。EK-Assistant不在客户端复刻这个阈值逻辑（会漂移），**决策：backend加一个薄薄的只读接口把这个函数包出来**——函数本身就是几条简单阈值判断，包接口是小时级工作量，不设"客户端近似判断"的兜底方案。这是MVP唯一需要backend配合的必需改动，其余全部复用现存接口。
...
| `check_kol_quality` | 只读，自动执行 | **新增**：薄封装`validateKolQuality()`的只读GET接口（唯一backend必需改动） |
...
| 评论反馈分析、效果归因（场景5） | A或fast-follow | 见开放问题 |
...
- 场景5（评论反馈分析+效果归因）是并入第一期MVP，还是作为第一期上线后的快速迭代（fast-follow）？它比前4个场景多需要backend一点新工作量（评论拉取+导出接口），值不值得让第一期上线时间往后挪，需要拍板。
